...
class Pyramid:
    def __init__(self, shader, base_width=0.2, base_height=0.2, height=0.3):
        ...
        self.vertices = np.array([
            ...
            [-base_width / 2, -height, -base_height / 2],  # Bottom-left
            [base_width / 2, -height, -base_height / 2],   # Bottom-right
            [base_width / 2, -height, base_height / 2],    # Top-right
            [-base_width / 2, -height, base_height / 2],   # Top-left
        ...
        # Indices remain the same as they define the connectivity
        ...
        # Updated normals for the new orientation
        self.normals = np.array([
            [0.0, -1.0, 0.0],  # Base normal (downward)
            # Side normals (approximate)
            [-0.7071, 0.7071, 0.0],  # Left side
            [0.7071, 0.7071, 0.0],   # Right side
            [0.0, 0.7071, 0.7071],   # Front side
            [0.0, 0.7071, -0.7071],  # Back side
        ], dtype=np.float32)
    ...
    def setup(self):
        # Setup VAO with VBOs for vertices, normals, and colors
        ...
        # Adjusted view matrix to look at the pyramid from a better angle
        self.view = glm.lookAt(
            glm.vec3(0.5, 0.5, 0.5),  # Camera position
        ...
        # Adjusted light position for new orientation
        light_pos = np.array([0.5, 1.0, 0.5], dtype=np.float32)
